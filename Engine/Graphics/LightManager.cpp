...
DirectionalLight::DirectionalLight() : color(Vector3::one), direction(Vector3::down), intensity(1.0f) {}
...
PointLight::PointLight() : color(Vector3::one), position(Vector3::zero), intensity(1.0f), radius(1.0f), decay(1.0f) {}
...
SpotLight::SpotLight() :
    color(Vector3::one), position(Vector3::zero), direction(-Vector3::unitY),
    intensity(1.0f), distance(1.0f), angle(60.0f * Math::ToRadian),
