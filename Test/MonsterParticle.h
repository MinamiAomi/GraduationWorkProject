...
public:
	...
	void Initialize();
	void Update();
	void EmitBatParticle(const Vector3& position);
	void EmitGhostParticle(const Vector3& position);
	void Emit();
...
public:
	std::shared_ptr<Material> batMaterial_;
	std::shared_ptr<Material> ghostMaterial_;
