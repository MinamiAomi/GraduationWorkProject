...
void LightDeviceInput::Update()
{
	...
	bool currentState = rawButtonPressed_.load();

	if (wasPressedSinceLastUpdate_.exchange(false)) {
		buttonPressed_ = true;
	}
	else {
		buttonPressed_ = currentState;
	}
}
...
void LightDeviceInput::CommunicationLoop() {
	...
	while (isRunning_.load()) {
		...
		if (data.size() >= 5) {
			...
			if (data[4] == "T") {
				buttonPressed = true;
				wasPressedSinceLastUpdate_.store(true); 
			}
			else {
				buttonPressed = false;
			}
		...
		rawButtonPressed_.store(buttonPressed);
...
bool LightDeviceInput::IsButtonRelease() const {
	return buttonPressed_ == false && buttonPressedPrev_ == true;
}
